...
export class DashboardController {
  ...
  @Get('attendance-trend') trend(@CurrentUser() user: RequestUser, @Query('months') months?: string) { const selectedMonths = optionalQueryNumber(months) ?? 6; if (!Number.isInteger(selectedMonths) || selectedMonths < 1 || selectedMonths > 12) throw new BadRequestException('Months must be an integer between 1 and 12'); return this.dashboard.attendanceTrend(user, selectedMonths); }
