...
async function main() {
  ...
  for (let index = 0; index < employees.length; index += 1) {
    ...
    const row = await prisma.employee.create({
      data: {
        ...
        employmentType: index === 5 ? EmploymentType.CONTRACT : EmploymentType.FULL_TIME,
        employmentStatus: index === 8 ? EmploymentStatus.ON_LEAVE : EmploymentStatus.ACTIVE,
  ...
  for (let employeeIndex = 0; employeeIndex < employeeRows.length; employeeIndex += 1) {
    for (let day = 1; day <= Math.min(now.getDate(), 28); day += 1) {
      ...
      const status = pattern === 0 ? AttendanceStatus.LEAVE : pattern === 1 ? AttendanceStatus.WORK_FROM_HOME : pattern === 2 ? AttendanceStatus.LATE : pattern === 3 ? AttendanceStatus.ABSENT : AttendanceStatus.PRESENT;
      await prisma.attendanceRecord.create({
        data: {
          employeeId: employeeRows[employeeIndex].id, date,
          checkIn: new Set<AttendanceStatus>([AttendanceStatus.PRESENT, AttendanceStatus.LATE, AttendanceStatus.WORK_FROM_HOME]).has(status) ? dateAt(date, status === AttendanceStatus.LATE ? 10 : 9, status === AttendanceStatus.LATE ? 18 : 4) : null,
          checkOut: new Set<AttendanceStatus>([AttendanceStatus.PRESENT, AttendanceStatus.LATE, AttendanceStatus.WORK_FROM_HOME]).has(status) ? dateAt(date, 18, 12) : null,
          workHours: status === AttendanceStatus.PRESENT ? 8.13 : status === AttendanceStatus.LATE ? 7.9 : status === AttendanceStatus.WORK_FROM_HOME ? 8.0 : null,
          status,
          notes: status === AttendanceStatus.WORK_FROM_HOME ? 'Approved remote day' : undefined,
        },
      });
    }
  }
